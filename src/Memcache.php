...
class MemcacheSASL
{
    ...
    {
        $this->_fp = stream_socket_client(config('aliyun.memcache.Host') . ':' . config('aliyun.memcache.Port'));
        $this->_send(array(
                    'opcode' => 0x21,
                    'key' => 'PLAIN',
                    'value' => '' . chr(0) . config('aliyun.memcache.UserName') . chr(0) . config('aliyun.memcache.PassWord')
                    ));
        $data = $this->_recv();
        if ($data['status']) {
